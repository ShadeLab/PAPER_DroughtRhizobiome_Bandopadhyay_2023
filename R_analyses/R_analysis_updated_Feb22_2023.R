...
ggplot(data=df, aes(x=Index, y=LibrarySize, color=Sample_or_Control)) + geom_point() ##Supplemental Figure S3
...
ggplot(data=df.pa, aes(x=pa.neg, y=pa.pos, color=contaminant)) + geom_point() +
  xlab("Prevalence (Negative Controls)") + ylab("Prevalence (True Samples)") ##Supplemental Figure S4
...
col<- cyan4
rarecurve<-rarecurve(t(otu_table(phyloseq_merged1)), step=20, label = FALSE, col="cyan4") ## Supplemental Figure S5
...
###boxplots for ratio threshold, SW
...
##boxplot of ratio threshold, bean
...
##generate plots 1 and 2 again with threshold >=1 
...
library(vegan)
library(dplyr)
rarecurve(t(otu_table(phyloseq_merged)), step=20, label = FALSE, col="cyan4") ##Supplemental Figure S9
...
metadata<- read.csv("bean-ww-planted-unplanted.csv") ##change metadata file as needed between bean and switchgrass
...
write.csv(phyloseq_class1, "bean_drought_phyloseq_class1_newlabel.csv")

write.csv(phyloseq_class1, "bean_ww_phyloseq_class1_newlabel.csv")
...
ggsave(filename = "dotplotclass_bean_dr_NicoCodeFINAL_newlabel_no1percent.tiff", plot = plot_dotplot_beandrought,
       width = 17 ,
       height = 16, units = c("cm"),
       dpi = 200)

ggsave(filename = "dotplotclass_bean_ww_NicoCodeFINAL_newlabel_no1percent.tiff", plot = plot_dotplot_beanww,
       width = 17 ,
       height = 16, units = c("cm"),
       dpi = 200)

##combined dot plot
my_plot_list <- list(plot_dotplot_beandrought, plot_dotplot_beanww)
plot_combined <- ggarrange(plotlist = my_plot_list, labels=c("A", "B"),
                           common.legend = TRUE, heights=10, legend="right") + bgcolor("white") 


plot_combined

## Figure 5
ggsave(filename = "bean_dr_ww_dotplot_final_no1percent.tiff", plot = plot_combined,
       width = 22,
       height = 18, units = c("cm"),
       dpi = 300)
...
reg1 <- lm(similarity~planted_y/days -1, data=dr_bean); summary(reg1) 
...
plot(result_bean_drought) #Figure S20
...
plot(result_metab_sample) #Figure S16A
...
##plot
plot<- ggplot(data, aes(x=harvest,y=final_conc,shape=drought,color=planted)) + theme_classic()+ scale_color_manual(values=c("#39568CFF", "#55C667FF")) +
  scale_shape_manual(values=c(1,16))+ #scale_size(range = c(1,10), # point size range
  #label = scales::dollar)+
  #expand_limits(shape = seq(2, 6, by = 1))+
  #scale_alpha_manual(values = c(2,3,4,5,6))+
  geom_point(aes(size=harvest)) +  scale_size_manual(values = c(1,2,3,4,5,6))+ #ylim(0.1, 0.6)+ #geom_smooth(method = lm, se=FALSE, aes(group=summarise), inherit.aes=TRUE) + #ggtitle("Bean")
  facet_grid(~crop)+theme(
    strip.text.x = element_text(
      size = 16
    ))+
...
ggsave(filename = "DNA_conc.tiff", plot = plot,
       width = 20,
       height = 15, units = c("cm"),
       dpi = 300)
...
##Figure 4A, D was generated by collaborator using software noted in methods section of MS, Inkscape was used to combine plots
##Figure 6, Inkscape used to edit and combine plots
...
##Figure S15 provided by collaborator
