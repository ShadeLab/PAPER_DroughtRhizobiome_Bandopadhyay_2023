...
plot<- ggplot(data=df, aes(x=Index, y=LibrarySize, color=Sample_or_Control)) + geom_point() ##Supplementary Fig. 8
...
plot<- ggplot(data=df.pa, aes(x=pa.neg, y=pa.pos, color=contaminant)) + geom_point() +
  xlab("Prevalence (Negative Controls)") + ylab("Prevalence (True Samples)") ##Supplementary Fig. 8
...
col<- cyan4
rarecurve<-rarecurve(t(otu_table(phyloseq_merged1)), step=20, label = FALSE, col="cyan4") ## Supplementary Fig. 8
...
##Figure 6 Nat Comms edit: did not use these edits when revising, submitted the original version above
...
bb<-c(0.320,0.250,0.200,0.150,0.100,0.050,0.001) # define breaks.
ll<-c("0.320","0.250","0.200","0.150","0.100","0.050","0.001") # labels. ##all samples representation checked for bean planted drought and watered such that the range does not miss any samples
plot_dotplot_beandrww<-
  ggplot(phyloseq_class2,aes(x=harvest,y=Class,size=Abundance,color=Abundance))+
  geom_beeswarm(priority="ascending",groupOnX=T, cex=2.5, side=1, corral="gutter")+
  #geom_circle(inherit.aes=TRUE)+ coord_fixed()+ ##geom_circle seems to be working only with numerical x axes
  #geom_jitter(aes(size=Abundance))+
  #geom_violin()+
  #geom_boxplot()+
  scale_x_discrete(limits = c("Day 0","Day 2","Day 3", "Day 4","Day 5", "Day 6"))+
  facet_grid(~summarise_new,scales="free")+ 
  ...
  #scale_colour_gradient(low="#132B43",
                        #high="#56B1F7",limits=c(0.001,0.3),breaks=bb)+
  scale_colour_viridis(limits=c(0.001,0.320),breaks=bb)+
  scale_size_continuous(limits=c(0.001,0.320),labels=ll,breaks=bb, name="Mean Relative\nAbundance")+
  theme(axis.title.x = element_blank()) + theme(axis.text.x = element_text(size=9, angle = 90, vjust = 0.5, hjust=1))+
  #ggtitle("Bean Drought")+
  theme(plot.title = element_text(hjust = 0.5))+
  labs(color="Mean Relative\nAbundance")+
...
#Figure 6
ggsave(filename = "Fig6_Apr7_2024.tiff", plot = plot_dotplot_beandrww,
       width = 40 ,
       height = 16, units = c("cm"),
       dpi = 300)
...
library(viridis)
library(ggplot2)
sw_dr_BC_time<- read.csv("bean-sw-dr-ww-BC-time-merge-edited.csv") ##combine all datasheets obtained above
View(sw_dr_BC_time)
...
##adding overlaying geom points 
sw_soilmoisture$crop<- c("Switchgrass")
bean_soilmoisture$crop<- c("Bean")
combine<- rbind(sw_soilmoisture, bean_soilmoisture)
combine1<- combine %>% mutate(measure="Gravimetric Soil Moisture (%)")
combine2<- combine1 %>% mutate(value=percentmoisture)
combine2$Treatment <- factor(combine2$Treatment,levels = c("pre-drought","drought","watered"), ordered=TRUE)
p1<-ggplot(data_final_moisture,aes(Harvest,value,fill=Treatment)) + geom_errorbar(limits, position = dodge, width = 0.25)+
  ...
  geom_point(data=combine2, mapping=aes(x=Harvest, y=value, shape= Treatment), position=position_dodge(width=0.9))+
  theme_classic()+
  facet_grid( crop~ measure+Type, scales="free_y" ) + ylim(0,111)+
...
sw_shootmass$crop<- c("Switchgrass")
bean_shootmass$crop<- c("Bean")
combine3<- rbind(sw_shootmass, bean_shootmass)
combine4<- combine3 %>% mutate(measure="Shoot Biomass (g)")
combine5<- combine4 %>% mutate(value=ShootMass)
combine5$Treatment <- factor(combine5$Treatment,levels = c("pre-drought","drought","watered"), ordered=TRUE)
combine5$Type<- c("planted")
p2<-ggplot(data_final_biomass,aes(Harvest,value,fill=Treatment)) + geom_errorbar(limits, position = dodge, width = 0.25)+
  geom_bar(aes(fill=Treatment),position="dodge",stat="identity")+ 
  geom_point(data=combine5, mapping=aes(x=Harvest, y=value, shape= Treatment), position=position_dodge(width=0.9))+
  theme_classic()+
...
##Nat Comms rev Final Figure 2 with overlaying points
ggsave(filename = "bean_sw_moisture_biomass_new_colorblindfriendly_final_newlabel_Apr22_2024.tiff", plot = plot_combined,
       width = 22,
       height = 13, units = c("cm"),
       dpi = 300)

###Figure 2 edits as per Nat Comms specs: tried box plots but did not use them in the final version

library(tidyverse)

#Grav moisture content
sw_soil_moisture<- read.csv("Switchgrass_soil.csv", row.names=1)
View(sw_soil_moisture)

colnames(sw_soil_moisture)<-c("Harvest","Treatment","Type","EnvMass","EnvWetMass","EnvDryMass")
sw_soil_moisture$percentmoisture<-((sw_soil_moisture$EnvWetMass-sw_soil_moisture$EnvMass)-(sw_soil_moisture$EnvDryMass-sw_soil_moisture$EnvMass))/(sw_soil_moisture$EnvDryMass-sw_soil_moisture$EnvMass)*100 
sw_soil_moisture<- sw_soil_moisture %>% mutate(crop="Switchgrass")

bean_soil_moisture<- read.csv("Bean_soil.csv", row.names=1)
View(bean_soil_moisture)

colnames(bean_soil_moisture)<-c("Harvest","Treatment","Type","EnvMass","EnvWetMass","EnvDryMass")
bean_soil_moisture$percentmoisture<-((bean_soil_moisture$EnvWetMass-bean_soil_moisture$EnvMass)-(bean_soil_moisture$EnvDryMass-bean_soil_moisture$EnvMass))/(bean_soil_moisture$EnvDryMass-bean_soil_moisture$EnvMass)*100 
bean_soil_moisture<- bean_soil_moisture %>% mutate(crop="Bean")

data_combined_moisture<- rbind(sw_soil_moisture, bean_soil_moisture)

data_combined_moisture<- na.omit(data_combined_moisture)

data_combined_moisture<- data_combined_moisture %>% mutate(measure="Gravimetric moisture content (%)")

data_combined_moisture$Treatment <- factor(data_combined_moisture$Treatment,levels = c("pre-drought","drought","watered"), ordered=TRUE)


##Shoot Biomass
sw_biomass<- read.csv("Switchgrass_biomass.csv", row.names=1)
View(sw_biomass)

colnames(sw_biomass)<-c("Harvest","Treatment","ShootMass")
sw_biomass<- sw_biomass %>% mutate(crop="Switchgrass")

bean_biomass<- read.csv("Bean_biomass.csv", row.names=1)
View(bean_biomass)

colnames(bean_biomass)<-c("Harvest","Treatment","ShootMass")

bean_biomass<- bean_biomass %>% mutate(crop="Bean")

data_combined_biomass<- rbind(sw_biomass, bean_biomass)

data_combined_biomass<- na.omit(data_combined_biomass)

data_combined_biomass<- data_combined_biomass %>% mutate(measure="Shoot biomass (g)")

data_combined_biomass$Treatment <- factor(data_combined_biomass$Treatment,levels = c("pre-drought","drought","watered"), ordered=TRUE)

##merge moisture and biomass data
data_final<- plyr::rbind.fill(data_combined_moisture, data_combined_biomass)
data_final<- data_final %>% mutate(value=ifelse(measure=="Gravimetric moisture content (%)", percentmoisture, ShootMass))

data_final<- data_final %>% mutate(planted=ifelse(is.na(Type), "planted", Type))

##plotting
library(ggh4x)
p1<-ggplot(data_final,aes(Harvest,value,fill=Treatment)) +
  geom_boxplot()+ 
  theme_classic()+
  ggh4x::facet_nested(crop~ measure+planted, scales="free_y", independent="y") +
  theme(panel.background = element_blank())+ scale_x_discrete(limits = c("Day 0","Day 2","Day 3","Day 4","Day 5", "Day 6"))+
  scale_fill_manual(values=c("#D55E00","#009E73","#56B4E9")) + theme(axis.title.x = element_text(size=15)) + theme(axis.text.x = element_text(size=12, angle = 90, vjust = 0.5, hjust=1))+
  guides(fill = guide_legend(keywidth = 1, keyheight = 1, ncol=1)) + ylab ("")+ xlab("") + theme(axis.text.y=element_text(size=12)) + 
  theme(legend.title = element_text(size=15))+ theme(legend.text = element_text(size=12))

ylim <- range(data_final$value[data_final$measure == "Gravimetric moisture content (%)"])
p2<- p1 + facetted_pos_scales(
  y = list(
    measure == "Gravimetric moisture content (%)" ~ scale_y_continuous(limits=ylim)
  )
)

ggsave(filename = "bean_sw_moisture_biomass_Apr7_2024.tiff", plot = p2,
       width = 20,
       height = 15, units = c("cm"),
       dpi = 300)
...
plot_heatmap_bean_unplanted_drought <- pheatmap::pheatmap(matrix_max_standardize_bean, ##if plotting sample by sample and not the means then use matrix_max_standardize_sw
                                                        cluster_cols = F, border_color = "white", na_col = "white", clustering_distance_rows="euclidean",clustering_method = "complete",
                                                        #main="Bean indicator species (Unique to planted+drought condition)",
                                                        color = colors2,
                                                        breaks = bk2
...
##Supplementary Figure 1C, save as PDF                  
...
##Figure 3 was generated by collaborator Xingxing Li





##Supplementary Materials figure notes
##Supplementary Fig. 6 was experiment images 
##Supplementary Fig. 7 was generated by GLBRC communications team
##Supplementary Fig. 1A was generated by collaborator using software noted in methods section of MS, Inkscape was used to combine plots
##Supplementary Fig. 2 was generated by collaborator using software noted in methods section of MS
